	...
	public class UICommand : MonoBehaviour {
			...
			if(building.OnceLanded == false) {
				...
				if(ct == CommandType.Info) {
					...
					if(BEWorkerManager.instance.WorkerAvailable()) {
						...
						UIDialogMessage.Show("Todos os servidores trabalhando no momento", "Ok", "Nenhum Servidores disponivel");
					}
				}
				else if(ct == CommandType.UpgradeCancel) {
					UIDialogMessage.Show("Cancelar melhorias atuais?", "Sim,Não", "Cancelar Melhorias ?", null, (result) => { MessageBoxResultUpgradeCancel(result); } );
				}
					...
					if(SceneTown.Gem.Target() >= FinishGemCount) {
						...
						UIDialogMessage.Show("Você precisa de mais SuperCorações para finalizar o trabalho imediatamente", "Ok", "Precisa de mais SuperCorações");
					}
				}
		...
		public void _Show(Building script) {
			...
			if(building.OnceLanded == false) {
				...
				AddButton(Resources.Load<Sprite>("Icons/Multiplication"), "Cancelar", PayType.None, 0, CommandType.CreateCancel);
			}
			else {

				// if building is not in creation, show 'info' button
				if(building.Level != 0)
					AddButton(Resources.Load<Sprite>("Icons/Info"), "Informação", PayType.None, 0, CommandType.Info);
				...
				if(building.defNext != null) {
					...
					if(building.InUpgrade) {
						if(!building.UpgradeCompleted) {
							...
							AddButton(Resources.Load<Sprite>("Icons/Multiplication"), "Cancelar", PayType.None, 0, CommandType.UpgradeCancel);
							int	 	FinishGemCount = building.GetFinishGemCount();
							AddButton(Resources.Load<Sprite>("Icons/Gem"), "Finalizar\nAgora", PayType.Gem, FinishGemCount, CommandType.UpgradeFinish);
						}
					}
					else {
                        ...
                        // Se objeto selecionado for a prefeitura, nao exibir a opcao de upgrade
                        if (building.Type == 0)
                        {
                            // Nao Faz nada
                            // Debug.Log("Prefs Selecionada: " );
                        } else
                        {
                            // add 'upgrade' button
                            AddButton(Resources.Load<Sprite>("Icons/UpArrow"), "Atualizar", payType, BuyPrice, CommandType.Upgrade);
                        }
                ...
                // Opcao de treinamento dos itens
			...
			Info.text = TBDatabase.GetBuildingName(building.Type) + "  Nivel "+building.Level.ToString ();
