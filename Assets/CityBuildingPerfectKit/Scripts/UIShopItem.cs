	...
	public class UIShopItem : MonoBehaviour {
		...
		public void Init(BuildingType _bt) {
			...
			if(Count >= CountMax) {
				...
				if(NextTownLevel == -1) DisabledInfo.text = "Gere mais recursos";
				else 					DisabledInfo.text = "Atualizar a Prefeitura para o nivel "+NextTownLevel.ToString ()+" para construir mais";
			...
			BuildTimeInfo.text = "Tempo:";
			BuildTime.text = BENumber.SecToString(bd.BuildTime);
			BuildCountInfo.text = "Total:";
			BuildCount.text = Count.ToString ()+"/"+CountMax.ToString ();

            // Cor da Borda do Intem Loja
			Border.color = CountAvailable ? new Color32(36, 165, 58, 255) : new Color32(0, 116, 47, 255);
			Icon.sprite = Resources.Load<Sprite>("Icons/Building/"+bt.Name);
			Icon.color = CountAvailable ? new Color32(255,255,255,255) : new Color32(255, 255, 255, 255);
		}
		...
		public void Clicked() {
			...
			if(bt != null) {
				...
				if(CountAvailable && PriceAvailable && BEWorkerManager.instance.WorkerAvailable()) {
					...
					// show message box
					if(!CountAvailable)  		UIDialogMessage.Show("Atualize a Prefeitura para receber mais recursos", "Ok", "Gere mais recursos");
					else if(!PriceAvailable)  	UIDialogMessage.Show("Você precisa de mais recursos ", "Ok", "Erro");
					else 						UIDialogMessage.Show("Todos os colaboradores estão ocupados", "Ok", "Erro");
				}
			}
