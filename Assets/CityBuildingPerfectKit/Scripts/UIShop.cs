	...
	public class UIShop : MonoBehaviour {
		...
		void _Show (ShopType type) {
			...
			if(eType == ShopType.Normal) {
				...
				// fill first tab
				bt.Clear ();
				bt.Add (TBDatabase.GetBuildingType(11)); // casas 
				bt.Add (TBDatabase.GetBuildingType(12));// ------
				bt.Add (TBDatabase.GetBuildingType(13));// casas
				bt.Add (TBDatabase.GetBuildingType(14));// conjuntos habitacionais
				FillContents(0, "Moradia", bt);

				// fill second tab
				bt.Clear ();
				bt.Add (TBDatabase.GetBuildingType(15));// casas maes
				bt.Add (TBDatabase.GetBuildingType(16));// escolas municipais
				bt.Add (TBDatabase.GetBuildingType(17));// ubs
				bt.Add (TBDatabase.GetBuildingType(18));// internet publica
				bt.Add (TBDatabase.GetBuildingType(10));
				//bt.Add (TBDatabase.GetBuildingType(9));
				//bt.Add (TBDatabase.GetBuildingType(10));
				FillContents(1, "Serviços", bt);
