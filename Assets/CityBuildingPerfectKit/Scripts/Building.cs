...
namespace BE {
	...
	public class Building : MonoBehaviour {
		...
		void Update () {
			...
			if(Landed && !InUpgrade && (def != null) && (def.eProductionType != PayType.None)) {
				...
				if(Collectable) {
                    ...
                    //mudar a cor do celetor na tela
			...
			if(InUpgrade) {
				...
				// update ui info
				uiInfo.TimeLeft.text = UpgradeCompleted ? "Completo!" : BENumber.SecToString(Mathf.CeilToInt(UpgradeTimeLeft));
		...
		public int GetFinishGemCount() {
            Debug.Log("Descontar Super Coracao");
			if(!InUpgrade || UpgradeCompleted) return 0;

			int	 	FinishGemCount = ((int)UpgradeTimeLeft+1)/30;
            if (FinishGemCount < 2)
            {
                Debug.Log("Recalculando novo Valor");
                return FinishGemCount + 3;
            } else
            {
		...
		public void Init(int type, int level) {
			...
			int displayLevel = (level == 0) ? 0 : level;
			...
			uiInfo.Level.text = "Nivel "+displayLevel.ToString ();
		...
		public void Move(int TileX, int TileZ) {
			...
			ground.Move(gameObject, tilePos, tileSize);
			//Debug.Log ("Move Pos: "+Landed.ToString ()+" "+ TileX.ToString()+","+ TileZ.ToString());
			CheckLandable();
			UpjustYByState();
		}
		...
		public bool Upgrade() {
			...
			if(payTypeReturn != PayType.None) {
				if(payTypeReturn == PayType.Gold) 			UIDialogMessage.Show("Moedas Insunficiente", "Ok", "Erro");
				else if(payTypeReturn == PayType.Elixir) 	UIDialogMessage.Show("Corações Insuficiente", "Ok", "Erro");
				else if(payTypeReturn == PayType.Gem) 		UIDialogMessage.Show("Recursos Insuficiente", "Ok", "Erro");
				else {}
		...
		public void UIFillProgress(ProgressInfo progress, BDInfo type) {
			...
			if(type == BDInfo.CapacityGold) {
				...
				progress.textInfo.text = "Capacidade : "+Capacity[(int)PayType.Gold].ToString("#,##0")+"/"+def.Capacity[(int)PayType.Gold].ToString ("#,##0");
				...
				progress.textInfo.text = "Capacidade : "+Capacity[(int)PayType.Elixir].ToString("#,##0")+"/"+def.Capacity[(int)PayType.Elixir].ToString ("#,##0");
				progress.imageMiddle.fillAmount = 0.0f;
				...
				progress.textInfo.text = "Capacidade : "+Production.ToString("#,##0")+"/"+def.Capacity[(int)def.eProductionType].ToString ("#,##0");
				progress.imageMiddle.fillAmount = 0.0f;
				progress.imageFront.fillAmount = Production/(float)(def.Capacity[(int)def.eProductionType]);
			}
			else if(type == BDInfo.ProductionRate) {
				progress.imageIcon.sprite = TBDatabase.GetPayTypeIcon(def.eProductionType);
				progress.textInfo.text = "Taxa de Produção : "+((int)(def.ProductionRate*3600.0f)).ToString("#,##0")+" por Hora";
				progress.imageMiddle.fillAmount = 0.0f;
				...
				progress.textInfo.text = "Satisfação : "+def.HitPoint.ToString("#,##0")+"/"+def.HitPoint.ToString("#,##0");
				progress.imageMiddle.fillAmount = 0.0f;
				...
				progress.textInfo.text = "Capacidade de Armazenamento : "+Capacity[(int)payType].ToString("#,##0")+"/"+def.Capacity[(int)payType].ToString ("#,##0");
				progress.imageMiddle.fillAmount = 0.0f;
		...
		public void UIFillProgressWithNext(ProgressInfo progress, BDInfo type) {
			...
			if(type == BDInfo.CapacityGold) {
				progress.imageIcon.sprite = TBDatabase.GetPayTypeIcon(PayType.Gold);
				progress.textInfo.text = "Capacidade : "+def.Capacity[(int)PayType.Gold].ToString ("#,##0")+"+"+(defNext.Capacity[(int)PayType.Gold]-def.Capacity[(int)PayType.Gold]).ToString ("#,##0");
				...
				progress.imageIcon.sprite = TBDatabase.GetPayTypeIcon(PayType.Elixir);
				progress.textInfo.text = "Capacidade : "+def.Capacity[(int)PayType.Elixir].ToString ("#,##0")+"+"+(defNext.Capacity[(int)PayType.Elixir]-def.Capacity[(int)PayType.Elixir]).ToString ("#,##0");
				...
				progress.imageIcon.sprite = TBDatabase.GetPayTypeIcon(def.eProductionType);
				progress.textInfo.text = "Capacidade : "+def.Capacity[(int)def.eProductionType].ToString ("#,##0")+"+"+(defNext.Capacity[(int)defNext.eProductionType]-def.Capacity[(int)def.eProductionType]).ToString ("#,##0");
				...
				progress.imageIcon.sprite = TBDatabase.GetPayTypeIcon(def.eProductionType);
				progress.textInfo.text = "Taxa de Produção : "+((int)(def.ProductionRate*3600.0f)).ToString("#,##0")+"+"+((int)((defNext.ProductionRate-def.ProductionRate)*3600.0f)).ToString("#,##0")+" por Hora";
				progress.imageMiddle.fillAmount = (defNext == null) ? 0.0f : defNext.ProductionRate/defLast.ProductionRate;
				progress.imageFront.fillAmount = def.ProductionRate/defLast.ProductionRate;
			}
			else if(type == BDInfo.HitPoint) {
				progress.imageIcon.sprite = Resources.Load<Sprite>("Icons/Heart");
				progress.textInfo.text = "Satisfação : "+def.HitPoint.ToString("#,##0")+"+"+(defNext.HitPoint-def.HitPoint).ToString("#,##0");
				...
				progress.imageIcon.sprite = TBDatabase.GetPayTypeIcon(payType);
				progress.textInfo.text = "Capacidade de Armazenamento : "+def.Capacity[(int)payType].ToString("#,##0")+"+"+(defNext.Capacity[(int)payType]-def.Capacity[(int)payType]).ToString ("#,##0");
