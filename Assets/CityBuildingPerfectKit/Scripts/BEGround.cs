...
namespace BE {
	...
	public class BEGround : MonoBehaviour {
		...
		public bool IsVacant(Vector2 tilePos, Vector2 tileSize) {
            ...
            if (DetectaColisao.podeCriar == false)
            {
                Debug.Log("Nao Pode Criar");
                return false;
                
            }
            //Debug.Log("Verificar se Colidiu");
					...
					if(AStar.tiles != null) {
						...
						for(int yy=0 ; yy < SubGridSize ; ++yy) {
							for(int xx=0 ; xx < SubGridSize ; ++xx) {
                                ...
                                if (((x == 0) && (xx == 0)) || ((x == (int)bd.tileSize.x - 1) && (xx == SubGridSize - 1)) || ((y == 0) && (yy == 0)) || ((y == (int)bd.tileSize.y - 1) && (yy == SubGridSize - 1)))
		...
		// retorna uma lista de Building pelo tipo
		public List<Building> GetBuildingsByType(int BuildingType) {
